...
func _enter_tree() -> void:
	...
	if JoypadManager.controller_active:
...
func _update_buttons() -> void:
	...
	for i in range(buttons.size()):
		...
		button.focus_mode = Control.FOCUS_ALL if controller_connected and controller_active else Control.FOCUS_NONE
		button.focus_behavior_recursive = FOCUS_BEHAVIOR_ENABLED if controller_connected else FOCUS_BEHAVIOR_DISABLED
		if i == 0 and (controller_awaiting_update and controller_connected):
			button.grab_focus()
